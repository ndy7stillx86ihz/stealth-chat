...
class ConnectionManager:
    def __init__(self):
        ...
        self.lock = Lock()
        self.publisher = EventManager(Event.CLIENT_DISCONNECTED, Event.CLIENT_REMOVED, Event.CLIENT_CONNECTED, Event.CLIENT_RECONNECTED)

    def add_client(self, client: ClientConnection):
        with self.lock:
            self.clients.append(client)
            if len(self.clients) > 1:
                self.publisher.notify(Event.CLIENT_CONNECTED, 'peer connected to chat')

    def remove_client(self, client: ClientConnection):
        with self.lock:
            self.clients.remove(client)
            client.close()
            self.publisher.notify(Event.CLIENT_DISCONNECTED, 'peer disconnected from chat')
